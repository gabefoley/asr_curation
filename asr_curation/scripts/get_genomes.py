                                    ...
                                    if 'CDSList' in prot and len(prot['CDSList']) > 0:
                                        ...
                                        for x in prot['CDSList']:
                                            ...
                                            if 'assembly' in x.attributes:
                                                ...
                                                if "GCA" in a_id:
                                                    if a_id.replace("GCA_", "GCF_") not in assembly_dict[entry_name]:
                                                        added = True
                                                        assembly_dict[entry_name].append(x.attributes['assembly'])
                                                else:
                                                    added = True
                                                    assembly_dict[entry_name].append(x.attributes['assembly'])
...
for chunked_dict in chunks(all_ids_dict, SIZE=800):
    ...
    original_df = add_to_df_from_dict(original_df, assembly_dict, 'Genomes_with_identical_proteins')
