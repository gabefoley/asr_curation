...
def parse_args(args):
    ...
    parser.add_argument("-s", "--seed", help="Set the random seed to an integer to make colour selection "
                                                    "reproducible")
    ...
    parser.add_argument("-m", "--match_from", help="Match from a specific column in the annotations file",
                        default="Name")
    parser.add_argument("-r", "--rotate", help="Internal nodes to rotate for visualisation", nargs='+', default=None)

    parser.add_argument("-w", "--whitespace_split", help="Split the column based on whitespace", action='store_true')
...
if __name__ == "__main__":
    ...
    tree, ts = tc.get_example_tree(tree = tree, color_dict = color_dict, annot_dict=annot_dict, col=col, rotate=parser.rotate)
