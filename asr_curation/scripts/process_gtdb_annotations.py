...
def check_for_gtdb_taxa_discrepency(gtdb_dict_list, taxa_rank='gtdb_phylum'):
    ...
    gtdb_genomes_list = [gtdb_genome[taxa_rank].split("__")[1] for gtdb_genome in gtdb_dict_list ]
...
def check_consistency_of_uniprot_gtdb_taxa(entry, gtdb_dict_list, 
	df_rank='Taxonomic_lineage_PHYLUM', 
	gtdb_rank = 'gtdb_phylum'):
    
    alias_dict = {"Actinobacteria" : "Actinobacteriota", 
    ...
    if len(gtdb_phyla) > 0:
        if entry.Taxonomic_lineage_PHYLUM == gtdb_phyla[0] or ((entry.Taxonomic_lineage_PHYLUM in alias_dict) and (alias_dict[entry.Taxonomic_lineage_PHYLUM].strip() == gtdb_phyla[0].strip())):
            return 'Consistent'
        else:
...
def get_highest_checkM_completeness(gtdb_dict_list):
    ...
    if len(gtdb_dict_list) > 0:
        highest_checkM_completeness = max([gtdb_genome['checkm_completeness'] for gtdb_genome in gtdb_dict_list])
...
def add_to_df_from_dict(df, dict_to_add, col_name, add_name = 'Entry'):
    ...
    df[col_name] = df.apply(
            lambda row: 
                dict_to_add[row[add_name]] if row[add_name] in dict_to_add else row[col_name],
            axis=1,
        )
...
#     print (gtdb_dict_list)
    ...
    gtdb_uniprot_consistency = check_consistency_of_uniprot_gtdb_taxa(row, gtdb_dict_list)
...
# Check the files
check_m_df = df[(df['Check_M_Highest']  < 70) & (df['Check_M_Highest'] != 'No_GTDB_info') ]

taxa_df = df[(df['Taxa_discrepency'] != 'Consistent') & (df['Taxa_discrepency'].notnull())]

uniprot_gtdb_df = df[(df['GTDB_Uniprot_consistency'] != 'Consistent') & (df['GTDB_Uniprot_consistency'].notnull())]
