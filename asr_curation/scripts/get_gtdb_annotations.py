...
original_df.Genomes_with_identical_proteins = original_df.Genomes_with_identical_proteins.apply(lambda x : literal_eval(x) if pd.notnull(x) else "not found")
...
assembly_dict_with_nulls = pd.Series(original_df.Genomes_with_identical_proteins.values,index=original_df.Entry.values).to_dict()
...
def get_gtdb_dict(assembly_dict):
    ...
    for name, entry_list in assembly_dict.items():
        ...
        for entry in entry_list:
            ...
            response = requests.get(f'https://gtdb.ecogenomic.org/api/v1/genome/summary/{entry}')
...
def check_for_gtdb_taxa_discrepency(gtdb_dict, taxa_rank='gtdb_phylum'):
    ...
    for prot_id, gtdb_genomes in gtdb_dict.items():
        ...
        gtdb_genomes_list = [gtdb_genome[taxa_rank].split("__")[1] for gtdb_genome in gtdb_genomes ]
...
def check_consistency_of_uniprot_gtdb_taxa(df, gtdb_dict, 
...
def get_highest_checkM_completeness(gtdb_dict):
    ...
    for prot_id, gtdb_genomes in gtdb_dict.items():
        ...
        if len(gtdb_genomes) > 1:
            ...
            highest_checkM_completeness = max([gtdb_genome['checkm_completeness'] for gtdb_genome in gtdb_genomes])
