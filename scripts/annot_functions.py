...
def add_lab_annotations(annot_df, filepath, seq_col='sequence'):
    ...
    if not df_diff.empty:
        for val in df_diff['accession'].values:
            if val in annot_df['accession'].values and val in lab_df['accession'].values:
                annot_seq = annot_df.loc[annot_df['accession'] == val, seq_col].values[0]
    ...
    if len(lab_df.columns) != len(lab_df.columns.str.replace('.1$', '').drop_duplicates()):
        ...
        if first_list:
            for pos, val in enumerate(first_list):
                if pos > len(second_list):
                    warnings.warn(f'Lab annotations have missing values - {accession}', UserWarning)

                if val == second_list[pos]:
                    continue
                else:
                    overwritten = True
                    warnings.warn(f'Lab annotations are overwriting values - {accession}', UserWarning)
