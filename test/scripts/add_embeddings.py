...
def calculate_embeddings(sequence, model, model_name, tokenizer):
    inputs = tokenizer(" ".join(sequence), return_tensors="pt", padding=True, truncation=True, max_length=512)
    ...
    embed_max_pool = last_hidden_state.max(dim=1).values.detach().numpy().flatten().tolist()
    hidden_states = outputs[2]
    last_two_layers = torch.cat((hidden_states[-1], hidden_states[-2]), dim=-1)
    embed_last_two_concat = last_two_layers.mean(dim=1).detach().numpy().flatten().tolist()
...
def process_and_store_embeddings(df, model_name, embedding_df_path='embeddings.pkl'):
    ...
    for idx, row in df.iterrows():
        ...
        existing_row = embedding_df[(embedding_df['sequence'] == sequence) & (embedding_df['model_name'] == model_name)]
        ...
        if not existing_row.empty:
            ...
            embedding_df = pd.concat([pd.DataFrame([new_row], embedding_df)], ignore_index=True)
