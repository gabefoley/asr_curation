...
def generate_dbscan_embeddings(
    ...
    for eps_param in dbscan_params_eps:
        for min_samples_param in dbscan_params_min_samples:
            ...
            df[
                f"dbscan_eps={eps_param}_minsamples={min_samples_param}"
...
def compare_categorical_distributions(df1, df2, column_name, alpha=0.05):
    ...
    contingency_table = (
        pd.concat([df1_counts, df2_counts], axis=1, keys=["df1", "df2"])
        .fillna(0)
        .astype(int)
    )
...
def generate_parameter_subsets(df, cols_to_check):
    ...
    for eps_param in dbscan_params_eps:
        for min_samples_param in dbscan_params_min_samples:
            noise_indices = set(
                df[
                    df[f"dbscan_eps={eps_param}_minsamples={min_samples_param}"] == -1
                ].index
            )
            ...
            subset_wo_noise = subset_wo_noise.drop(columns=["_merge"]).reset_index(
                drop=True
            )
            subset_wo_noise = subset_wo_noise[
                [col for col in subset_wo_noise.columns if not col.endswith("_df2")]
            ]
            ...
            if subset_wo_noise.shape[0] != 0:
                for column in cols_to_check:
                    ...
                    diff_column = compare_categorical_distributions(
                        df, noise_df, column, alpha
                    )
...
def generate_db_scan_coverage_plot(df, parameter_subsets, outpath):
    ...
    for i, eps_param in enumerate(dbscan_params_eps):
        for j, min_samples_param in enumerate(dbscan_params_min_samples):
            ...
            features = set(
                parameter_subsets.get(combination_name, [])
            )  # Get the set of features for the combination
            ...
            if features_frozen in colour_mapping:
                ...
                colour = plt.cm.tab20(
                    len(colour_mapping)
                )  # Use a different color for each unique feature set
    ...
    for features in sorted_features.keys():
        ...
        legend_labels.append(
            plt.Line2D(
                ...
                marker="o",
                color="w",
                label=[x for x in list(features)],
                markersize=10,
...
def generate_dbscan_coverage(df, embedding_col, outpath, skip_cols=[]):
    ...
    skip_cols += [
        ...
        "lineage_thermo",
        "thermo_bacteria",
        "lineage_thermo_no_metha",
        "model_name",
        "Prot_T5 Embed Encoded",
        "Prot_T5 Embed Mean",
        "Prot_T5 Embed CLS",
        "embeddings_encoded",
        "BRENDA",
    ...
    cols_to_check = [
        x for x in df.keys() if not any(x.startswith(term) for term in skip_cols)
    ]
