...
from ete3 import PhyloTree, TreeStyle, TextFace, add_face_to_node, SeqMotifFace, SequenceFace, NodeStyle, faces, \
    ImgFace, CircleFace, AttrFace
...
def highlight_tree_nodes(tree,
    ...
    for n in tree.traverse():
        ...
        if processable_node(n, highlight_nodes) or n.name in highlight_nodes or display_all_nodes:
            if n.name in highlight_nodes or display_all_nodes:
                ...
                if seq_pos:
                    ...
                    parentseq = [n.up.sequence[x] for x in seq_pos] if not n.is_root() else ""
                    ...
                    if seq_change and "".join(subseq) == "".join(parentseq):
                        ...
                        if seq_motif:
                            ...
                            S = SequenceFace(subseq, interactive=True, col_w=20, fsize=18)
                            n.add_face(S, 1, position="branch-bottom")
...
def ancestral_trace(tree, seqs_to_trace_to, seq_pos, seq_change=False):
    ...
    tree, ts = highlight_tree_nodes(tree, highlight_nodes=highlight_nodes, seq_change=seq_change, seq_pos=seq_pos)
