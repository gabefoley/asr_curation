...
if 'uniprot_ec_1_1_1_86' in snakemake.wildcards.dataset:
    ...
    aln_binding_pos = an.get_aligned_positions(align_df['Entry'].iloc[0],align_df['Sequence'].iloc[0], *ref_binding_pos)
    ...
    anc_df['Binding_positions_alignment_residues_ancestor'] = anc_df.apply(
        lambda row: an.get_amino_acids(row['Sequence_aligned'], *aln_binding_pos), axis=1)


    anc_df['Acidic_Binding_alignment'] = anc_df.apply(lambda row: an.check_sequence_for_acidic(row['Binding_positions_alignment_residues_ancestor']), axis=1)
